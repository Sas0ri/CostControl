...
@interface ProjectDrawBackCtr ()
@property (nonatomic, strong) NSArray* modelList;
@end

@implementation ProjectDrawBackCtr
...
- (void)_reloadData
{
	...
	for (NSDictionary* dic in array) {
		PDModel* model = [[PDModel alloc] initWithDic:dic];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (indexPath.row > 0 && indexPath.row < self.modelList.count+1 && indexPath.section == 0) {
		return 250;
	}
	return 44;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (indexPath.row > 0 && indexPath.row < self.modelList.count+1) {
		static NSString *CellIdentifier = @"DrawBackCell";
		DrawBackCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier forIndexPath:indexPath];
...
{
	PDModel* model = ((DrawBackCell*)cell).model;
...
- (NSArray *)submitArr
{
	...
	for (PDModel* model in self.modelList) {
