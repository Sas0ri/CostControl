...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.backgroundDivImage) {
        if (self.sectionTitles.count > 1) {
            CGFloat y = (self.height - self.selectionIndicatorHeight - self.backgroundDivImage.size.height)/2;
            for (NSInteger index = 1; index < self.sectionTitles.count; index++) {
                CGRect rect = CGRectMake(self.segmentWidth*index - self.backgroundDivImage.size.width/2, y, self.backgroundDivImage.size.width, self.backgroundDivImage.size.height);
                [self.backgroundDivImage drawInRect:rect];
            }
        }
    }
