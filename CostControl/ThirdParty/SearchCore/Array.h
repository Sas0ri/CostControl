...
{	
	...
	size_t** pIndexData;	  //索引空间首地址
	int pIndexNum;        //索引空间个数

	size_t* pDataEnd;

	void (*Append)(struct Array* A,size_t value);
	void (*Insert)(struct Array* A,size_t value,int pos);
	void (*Remove)(struct Array* A,int index);
	void (*Reset)(struct Array* A);
	size_t (*GetValue)(struct Array* A,int index);

}Array;

void ArrayInit(struct Array* A);
void ArrayAppend(Array* A,size_t value);
void ArrayInsert(Array* A,size_t value,int pos);
void ArrayRemove(Array* A,int index);
void ArrayReset(Array* A);
int ArrayReSize(Array* A);
size_t ArrayGetValue(Array* A,int index);
